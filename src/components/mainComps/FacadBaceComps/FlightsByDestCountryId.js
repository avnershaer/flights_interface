...
function FlightsByDestCountryId() {
  ...
  const handleApiCallClick = async () => {
    if (selectedCountryId) {
      try {
        const response = await FlightsDetailsByDestCountryId(selectedCountryId);
  ...
  return (
    <div className="FlightByDestCountryId-container">
    <div>  
      <div>
        <div>
          <span style={{ color: "white" }}>GET FLIGHTS DETAILS by DESTINATION COUNTRY ID</span>
        </div>  
        <br/>             
        Enter destination country id<br/>              
       
        <input type="number" placeholder="Destination Country ID" value={selectedCountryId} onChange={handleIdChange} min="1"/>
      </div>
...
export default FlightsByDestCountryId;