...
function DepartureFlightsByCountryId() {
  ...
  const handleApiCallClick = async () => {
    if (selectedCountryId) {
      try {
        const response = await DepartFlightsByCountryId(selectedCountryId);
  ...
  return (
    <div className="ById-container">
      ...
      <div>
        <div>
          <span style={{ color: "white" }}>GET NEXT 12 HOURS DEPARTURE FLIGHT DETAILS, by COUNTRY ID</span>
        </div>  
        <br/>             
        Enter Country id<br/>              
      ...
      <div className="send-id-bottun"  > 
        <button onClick={handleApiCallClick}>Get departure flights details</button>
      </div>
...
export default DepartureFlightsByCountryId;